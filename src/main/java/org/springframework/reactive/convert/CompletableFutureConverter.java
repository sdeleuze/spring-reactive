 ...
 */
public class CompletableFutureConverter implements ReactiveConverter {
	...
	public Boolean isCollection(TypeDescriptor sourceType, TypeDescriptor targetType) {
		if (CompletableFuture.class.isAssignableFrom(sourceType.getResolvableType().getRawClass()) ||
				CompletableFuture.class.isAssignableFrom(targetType.getResolvableType().getRawClass())) {
			return false;
		}
		return null;
	}
